...
public class WorldGuardCheck {
    ...
    public boolean WorldguardCheck(Location l) {
                    ...
                    for (int j = 0; j < r * 2 + 1; j++) {
                        for (int k = 0; k < r * 2 + 1; k++) {
                            ...
                            if (RandomCoords.getPlugin().config.getStringList("Regions").contains("allRegions")) {
                                ...
                                for (ProtectedRegion reg : RandomCoords.getPlugin().getWorldGuard().getRegionManager(l.getWorld()).getApplicableRegions(l)) {
                                    for (String regName : RandomCoords.getPlugin().config.getStringList("Regions")) {
